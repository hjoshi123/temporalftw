...
main() {
    ...
    if [ "$_path_set" = false ] && check_cmd "atlas"; then
        local _out="$(atlas version 2>/dev/null)"
    ...
    local head_response_status
    head_response_status=$(curl --head --silent --write-out '%{http_code}' --output /dev/null "$_url")
    if [ "$head_response_status" -eq 404 ] && [ "$ATLAS_VERSION" != "latest" ]; then
        err "The specified Atlas version does not exist or is no longer supported."
    fi
    ...
    cd "$_dir"
    ensure_silent curl -Lk -o "$_file" -A "$_ua" --fail "-#" "$_url" || back
    back
